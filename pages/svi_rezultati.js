...
export default function SveRezultati() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await axios("/api/svi_rezultati");
        setMatchesData(result.data);
        setLoading(false);
      } catch (err) {
        setError("Failed to load all results data");
        setLoading(false);
      }
    };
    fetchData();
  ...
  return (
    <>
      ...
      <div style={containerStyle}>
        {matchesData.map((round, roundIndex) => (
          <div key={roundIndex} style={{ marginBottom: "24px" }}>
            <h3 style={roundTitleStyle}>Kolo {round.roundNumber}</h3>
